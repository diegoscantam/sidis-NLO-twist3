...
\begin{equation}
    \begin{aligned}
        k^\mu &= xP^\mu + k_T^\mu+(k\cdot P)n^\mu\approx xP^\mu,\\
        p^\mu &= \frac{1}{z}P_h^\mu + p_T^\mu+(p\cdot P_h)m^\mu\approx \frac{1}{z}P_h^\mu + p_T^\mu,
    \end{aligned}
\end{equation}
where, as typical, we neglected contributions of order two or higher in the transverse momenta. Note that this approximation is consistent with the application of the collinear twist-3 formalism, since we will need only linear terms in $p_T$ to fully derive a factorized spin-dependent cross section involving twist-3 fragmentation functions. Note that since we deal with leading twist distribution functions only, we can neglect $k_T$ altogether and approximate the partonic momentum $k$ as exactly collinear to the nucleon momentum $P$.
...
\begin{equation}
    \int \dd\Pi_{{\rm{LIPS}}_n}=\Theta(n-1)\prod_{i=1}^n \int \frac{\dd^d r_i}{(2\pi)^{d-1}}\delta^+(r_i^2) \,\delta^{(d)}\left(q+k - p - \sum_{i=1}^n r_i\right),
\end{equation}
...
\begin{figure}[h!] % Use [h!] to try and keep it here
    ...
    \begin{minipage}[b]{0.6\textwidth} % [b] aligns bottoms, 0.48*textwidth for width
        ...
        %\label{fig:second}
    \end{minipage}
    \caption{3-parton fragmentation contributions to the hadronic tensor for $n=1$ unobserved partons. We show $W_{\mu\nu}^{q \to qg}$ (top). $W_{\mu\nu}^{q \to q\bar{1}}$ (bottom)} % Optional overall caption
    \label{fig:W_n1_3parton}
\end{figure}

All momenta are now decomposed through the light-cone decomposition in the collinear hadron frame introduced before. Performing the (trivial) integrations over $\dd (k \cdot P)$ and $\dd (p\cdot P_h)$, one sees that the integral measures and the delta function arrange in the following way
\begin{equation}
    \begin{aligned}
...
\begin{equation}
    a^\mu = (a\cdot n)P^\mu + (a \cdot P)n^\mu+a_T^\mu=(a\cdot m)P_h^\mu + (a \cdot P_h)m^\mu+a_T^\mu.
\end{equation} 
We stress that the separation of the particle momenta in such a way makes sense only in a frame where the nucleon and the hadron are collinear, i.e. the $PP_h$ frame used so far. In this case, the transverse vectors are indeed 2-dimensional and this kind of $(+,-,T)$ separation makes sense.
Our goal, though, is to study the differential cross section integrated over the transverse momentum of the observed hadron. However, in a collinear frame such as the one introduced so far, the momentum $P_h$ is fixed and it has no transverse component by construction (EQ REF). A possible approach would be to integrate over the transverse momentum $q_T$ of the virtual photon. This $q_T$ integration makes a clean separation between leptonic and hadronic parts impossible, since the leptonic tensor $L^{\mu\nu}$  will clearly depend on this transverse momentum. A common procedure is then to change frame in such a way that this separation is possible \cite{mulders_complete_1996}. It turns out that the Breit frame in which the virtual photon and the nucleon are collinear is an appropriate choice. In this new frame, the photon momentum does not have a temporal component (which is possible since in SIDIS $q^\mu$ is always a space-like vector). The hadron momentum now acquires a perpendicular component. We then construct the Lorentz transformation from the collinear hadron frame to the Breit frame. It turns out to be
\begin{equation}
...
ADD STUFF HERE ON DETAILS OF DERIVATION + CHECK WHAT MARC WRITES IN PAPER! 
