...
!   TSAs in Phperp integrated SIDIS 
!
!   Diego Scantamburlo, October 2025
!
!###############################################################################
...
! Useful for avoiding annoying common blocks later on
...
program sidis
    ...
    real*8 dsigmadxdydz, y_arr(0:nplot2),z_arr(0:nplot), x_arr(0:nplot),&
          ...
          x,y,z,muu, sigmaLO_unpolarized
    real*8 output,output2Q,outputQd2,mu,mu_up,mu_down
    real*8 x_arr_plot(3),z_arr_plot(3), weigthed_sum_f1D1
    integer do_this

    EXTERNAL sigmaLO_unpolarized,weigthed_sum_f1D1
    ...
    print *, "LO unp  = ",sigmaLO_unpolarized(x,y,z)  
...
SUBROUTINE sigmaNLO_unpolarized(xB,y,zh,muu,output)
    ...
    real*8  region_wv(4), sd,chi2a
    real*8  sigmaNLO_unpolarizedAUXwv

    EXTERNAL ALPHAS,sigmaNLO_unpolarizedAUXwv
...
function sigmaNLO_unpolarizedAUXwv(Y)
        ...
        real*8  xB_,y_,zh_, muu_, w,v, Y(*), alpha_running,&
               ALPHAS, weigthed_sum_f1D1, weigthed_sum_f1_times_D1g,&
               weigthed_sum_D1_times_f1g, zh, xB, yy, mu, Conv, CF, TF
        COMMON /xyzmu/ xB_,y_,zh_, muu_
        EXTERNAL ALPHAS,weigthed_sum_f1D1, weigthed_sum_f1_times_D1g,&
                  weigthed_sum_D1_times_f1g

        ! Factor from Gev^-2 to picobarn
        !Conv = 0.389379d0*(10.d0**(9.d0))
        ...
        !Get alpha_S(mu)
        alpha_running = ALPHAS(mu)
        
        w = Y(1)
        v = Y(2)

        ! Soft functions, sums of f1 & D1 for different channels
        softq2qOFwv = weigthed_sum_f1D1(xB/w,zh/v,mu)
        softq2qOFw = weigthed_sum_f1D1(xB/w,zh,mu)
        softq2qOFv = weigthed_sum_f1D1(xB,zh/v,mu)
        softq2q = weigthed_sum_f1D1(xB,zh,mu)

        softg2qOFwv = weigthed_sum_D1_times_f1g(xB/w,zh/v,mu)
        softg2qOFw = weigthed_sum_D1_times_f1g(xB/w,zh,mu)

        softq2gOFwv = weigthed_sum_f1_times_D1g(xB/w,zh/v,mu)
        softq2gOFv = weigthed_sum_f1_times_D1g(xB,zh/v,mu)
        

        ! prefactors for cross section and structure functions
        preNLO = (4d0 * pi * alpha_em * alpha_em /Q2/yy)
        preFUUT = (1d0-yy+yy*yy/2d0)
        preFUUL = (1d0-yy)

        
        sigmaNLO_unpolarizedAUXwv= &
            ! NLO corrections to FUUT structure function
            preNLO*preFUUT*(  &
            !!!!!!! q -> q channel !!!!!!
            !! delta delta 
            softq2q*(1d0 + CF*(alpha_running/2d0/pi)*(-8d0 -3d0 *dlog(mu*mu/Q2) ) )/(1d0-xB)/(1d0-zh)&
            !! delta 1 - w PROBLEM HERE !!!!!!!!!!!!!!!!!!!!!!!!!!1
            !+CF*(alpha_running/2d0/pi)*(   &
            !  (dlog(1d0-v)/(1d0-v))*(1d0+v*v)*((1d0/v)*softq2qOFv* -2d0*softq2q ) + 1d0 - v + &
            !  (1d0/(1d0-v))*((1d0+v*v)*(dlog(v)-dlog(mu*mu/Q2))*(1d0/v)*softq2qOFv  - 2d0*(-dlog(mu*mu/Q2))*softq2q  ) &
            !    )/(1d0-xB) &
            !+CF*(alpha_running/2d0/pi)*softq2q*(  (dlog(1d0-v)/(1d0-v))*( -2d0 ))/(1d0-xB) &
            !+CF*(alpha_running/2d0/pi)*softq2q*(  (-0.5*dlog(1d0-zh)*dlog(1d0-zh))*( -2d0 ))/(1d0-xB)/(1d0-zh) &
            !+CF*(alpha_running/2d0/pi)*softq2q*(  (1d0/(1d0-v))*(-2d0 ))/(1d0-xB) &
            !+CF*(alpha_running/2d0/pi)*softq2q*(  (-dlog(1d0-zh))*(-2d0 ))/(1d0-xB)/(1d0-zh) &
            ! delta 1 - v
            +CF*(alpha_running/2d0/pi)*(1d0/w)*softq2qOFw*(   &
              (dlog(1d0-w)/(1d0-w))*(1d0+w*w) +1d0 - w + &
              (1d0/(1d0-w))*(1d0+w*w)*(-dlog(w)-dlog(mu*mu/Q2)) &
                )/(1d0-zh) &
            +CF*(alpha_running/2d0/pi)*softq2q*(  (dlog(1d0-w)/(1d0-w))*( -2d0 ))/(1d0-zh) &
            +CF*(alpha_running/2d0/pi)*softq2q*(  (-0.5*dlog(1d0-xB)*dlog(1d0-xB))*( -2d0 ))/(1d0-zh)/(1d0-xB) &
            +CF*(alpha_running/2d0/pi)*softq2q*(  (1d0/(1d0-w))*(-2d0 ))/(1d0-zh) &
            +CF*(alpha_running/2d0/pi)*softq2q*(  (-dlog(1d0-xB))*(-2d0 ))/(1d0-zh)/(1d0-xB) &
            ! plus plus 
            +CF*(alpha_running/2d0/pi)*(1d0/v/w)*softq2qOFwv*(  &
            (1d0/(1d0-w)/(1d0-v))*(2d0*v*v*w*w - 2d0*v*v*w - 2d0*v*w*w + 4d0*v*w + v*v + w*w - 2d0*v - 2d0*w + 2d0 )) &
            -CF*(alpha_running/2d0/pi)*(1d0/v)*softq2qOFv*((1d0/(1d0-w)/(1d0-v))*(1d0 + v*v)) &
          ...
          !  +TF*(alpha_running/2d0/pi)*(1d0/w)*softg2qOFw*(&
          !    (w*w + (1d0-w)**2d0)*(  dlog(1.d0-w)-dlog(mu*mu/Q2)) + 2d0*w*(1d0 - w))/(1d0-zh)   &
          !  ! plus
            +TF*(alpha_running/2d0/pi)*(1d0/w/v)*softg2qOFwv* (1d0/(1d0-v))*&
              (  (w*w + (1d0-w)**2d0)*(v*v + (1d0-v)**2d0)/v ) &
            +TF*(alpha_running/2d0/pi)*(1d0/w)*softg2qOFw*(1d0/(1d0-v))*&
              (  -(w*w + (1d0-w)**2d0) )  &
            +TF*(alpha_running/2d0/pi)*(1d0/w)*softg2qOFw*(-dlog(1d0-zh))*&
              (  -(w*w + (1d0-w)**2d0) )/(1d0-zh)   &
           ! !!!!!! q -> g channel !!!!!!
            ! delta 1 - w
           ! +CF*(alpha_running/2d0/pi)*(1d0/v)*softq2gOFv*(&
           !   ((1d0+(1d0-v)**2)/v )*(  dlog( v*(1d0-v) ) -dlog(mu*mu/Q2)) + v)/(1d0-xB)   &
           ! ! plus
            +CF*(alpha_running/2d0/pi)*(1d0/w/v)*softq2gOFwv*(1d0/(1d0-w))*&
              ( (1d0 + v*v + w*w - 2d0*v*w*w - 2d0*v*v*w + 2d0*v*v*w*w)/v ) &
            +CF*(alpha_running/2d0/pi)*(1d0/v)*softq2gOFv*(1d0/(1d0-w))*&
              (  -(1d0 + v*v + 1d0 - 2d0*v - 2d0*v*v + 2d0*v*v)/v ) &
            +CF*(alpha_running/2d0/pi)*(1d0/v)*softq2gOFv*(-dlog(1d0-xB))*&
              (  -(1d0 + v*v + 1d0 - 2d0*v - 2d0*v*v + 2d0*v*v)/v )/(1d0-xB)  &
             ) &
            ! terms emerging at NLO contributing to FUUL structure function
            +preNLO*preFUUL* (  &
            CF*(alpha_running/2d0/pi)*(1d0/w/v)*(4d0*v*w)*softq2q &
            +TF*(alpha_running/2d0/pi)*(1d0/w/v)*(8d0*w*(1d0 - w))*softg2q  &
            +CF*(alpha_running/2d0/pi)*(1d0/w/v)*(4d0*w*(1d0 - v))*softq2g  )
            
            !sigmaNLO_unpolarizedAUXwv= preNLO*preFUUT*softg2qOFwv
