...
\end{equation}
and the splitting functions are found in Eq. 14 of Ref. \cite{Ma_2017}. 
...
\begin{equation}\label{eq:F_LT@NLO}
    ...
    \end{aligned}
\end{equation}
Crucially, we observe that collinear twist-3 factorization holds at NLO in QCD for these $P_{h\perp}$-integrated observables in SIDIS. Similarly to the unpolarized case, the structure functions are described as convolutions of perturbative hard scattering coefficients and twist-3 fragmentation functions. Here, the convolution involves not only the integrations over $w=x_B/x$ and $v=z_h/z$, but also an extra integral over the partonic variable $\zeta=z/z'$. For the interested reader, the explicit expressions for the coefficients can be found in Appendix \ref{appendix:polarized}.
