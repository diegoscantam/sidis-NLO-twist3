\chapter{Next-to-Leading Order QCD Analysis}\label{chap:NLO}
%alt+Z to format everything to column width!
...
PUT HERE CROSS SECTION WITH THE 4 STRUCTURE FUNCTIONS AS INTRO

\section{Unpolarized case}
We start the calculation at NLO in $\alpha_S$ by computing the unpolarized cross section at leading twist. This is a well-known result in the literature CITE, and we reproduce it here in order to illustrate the techniques that will be useful for the more complicated twist-3 calculation presented in the next section. As typical, two types of corrections contribute to the cross section at NLO: virtual corrections (vertex and self-energy graphs) and real corrections (additional unobserved partons in the final state).
...
\end{figure}
\noindent Since we are performing our pQCD calculation in light-cone gauge, we shall employ an appropriate technique to evaluate loop integrals that allows us to treat the denominator of the gluon propagator in this gauge. Notoriously, this $(r \cdot m)^{-1}$ term requires a careful treatment since it may lead to additional well-known \textit{light-cone divergences}, appearing when $r \cdot m \to 0$. Not surprisingly, it turns out that a light-cone decomposition of the loop momentum allows for a systematic procedure to integrate out the plus, minus and transverse components of the momentum. Since this technique is essential for calculating virtual corrections also at the twist-3 level, we present it here in detail for the simpler leading-twist case. The method is quite general and can be readily extended to higher-twist contributions. Let's start by calculating the vertex correction graph. The amplitude reads
...
\end{aligned}
\end{equation}
After $\beta$ contour integration, the denominator factors arrange as $\beta_1-\beta_j=C_{1j}(\vec r_\perp^2+\omega^2_{1j})$, with $j=0,2$ and $C_{1j},\omega^2_{1j}$ some functions of $\alpha$. Then, the integration over the transverse loop momentum can be readily performed since the integrand depends only on the square modulus of $r_\perp$ (after dropping odd power terms in the numerator since they vanish for symmetry). Adopting $d-2$ dimensional spherical coordinates, the angular integration is trivial and the radial integral turns out to be of the form
\begin{equation}
...
%    \end{aligned}
%\end{equation}
This procedure makes the $1/\epsilon$ poles explicit and leads to the well-known cancellation of infrared singularities between real and virtual contributions (commonly referred as infrared safety \cite{kinoshita_mass_1962}). We do indeed observe this cancellation of $1/\epsilon^2$ poles in our calculation.
...
\begin{equation}
\begin{aligned}
    f^q_{\text{bare}}(x,\mu^2)&= f^q_{\text{ren}}(x,\mu^2)+\frac{\alpha_S(\mu^2)}{2\pi}\frac{S_\epsilon}{\epsilon}\left[P_{qq} \otimes f^q_{\text{ren}}+P_{qg} \otimes f^g_{\text{ren}}\right](x,\mu^2)\\
    D^q_{\text{bare}}(z,\mu^2)&= D^q_{\text{ren}}(z,\mu^2)+\frac{\alpha_S(\mu^2)}{2\pi}\frac{S_\epsilon}{\epsilon}\left[P_{qq} \otimes D^q_{\text{ren}}+P_{gq} \otimes D^g_{\text{ren}}\right](z,\mu^2)
\end{aligned}
\end{equation}
...
\subsection*{Results}
We obtain the following unpolarized NLO result for the unpolarized structure functions, which is consistent with the literature \cite{de_Florian_1998}
\begin{equation}
    \begin{aligned}
        &F_{UU,T}(x_B,z_h,\mu^2)= x_B\sum_{q,\bar{q}} e_q^2 \Bigg\{f_1^q(x_B,\mu^2)D_1^q(z_h,\mu^2)\\
        &\quad+ \frac{\alpha_S(\mu^2)}{2\pi}\left[f_1^q \otimes \mathcal{C}_1^{q \to q} \otimes D_1^q +f_1^g \otimes \mathcal{C}_{1}^{g \to q} \otimes D_1^q+f_1^q \otimes \mathcal{C}_1^{q \to g} \otimes D_1^g\right](x_B,z_h,\mu^2) \Bigg\}\\
        &F_{UU,L}(x_B,z_h,\mu^2)=x_B \frac{\alpha_S(\mu^2)}{2\pi}\sum_{q,\bar{q}} e_q^2 \Big[f_1^q \otimes \mathcal{C}_L^{q \to q} \otimes D_1^q \\&\quad +f_1^g \otimes \mathcal{C}_L^{g \to q} \otimes D_1^q+f_1^q \otimes \mathcal{C}_L^{q \to g} \otimes D_1^g \Big](x_B,z_h,\mu^2)
    \end{aligned}
\end{equation}
where we introduced the double convolution $a\otimes b\otimes c$ defined as
\begin{equation}
    f \otimes \mathcal{C} \otimes D = \int_{x_B}^1 \frac{\dd w}{w}\int_{z_h}^1 \frac{\dd v}{v} f\left(\frac{x_B}{w},\mu^2\right) \mathcal{C}\left(w,v,\mu^2\right) D\left(\frac{z_h}{v},\mu^2\right)
\end{equation}
The functions $\mathcal{C}_1$ and $\mathcal{C}_L$ are the unpolarized NLO hard-scattering coefficients relative to the unpolarized structure functions $F_{UU,U}$ and $F_{UU,L}$ respectively. It is very interesting to note that the unpolarized structure function $F_{UU,L}$ is populated at NLO, while it is zero at LO. The explicit coefficients $\mathcal{C}_1$ and $\mathcal{C}_L$ are found in Appendix \ref{appendix:unpolarized}.
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{fig/VirtNLOTw3_reduced.pdf}
    \caption{Virtual hard scattering diagrams contributing to the NLO dynamical twist-3 $qg$ fragmentation channel.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.99\linewidth]{fig/RealNLOTw3.pdf}
...
When it come to the $q\bar{q}$ case, real emission of quarks is taken into account. The relevant diagrams are shown in Fig.~\ref{fig:Real NLO tw3}. We observe that the hadronic tensor is $\kappa$-gauge invariant and electromagnetic gauge invariant, since it is the only contribution in this channel. It also $1/\epsilon^2$ poles free, in agreement with the fact that there are no virtual corrections to cancel these double poles emerging from the phase space integration. In this sense, the $q\bar{q}$ channel is significantly simpler compared to the $qg$ one and we regard it as a "stand-alone" channel. This is particularly useful when it comes to renormalization, as we will see in the next section. If the IR behaviour of this channel is in agreement with the evolution equations of the relevant fragmentation functions, then we regard it as a strong clue that our calculation is consistent.

\subsection*{Renormalization}
Once all the terms are added and combined through the EoMR, we obtain gauge invariant partonic cross sections, free of $1/\epsilon^2$ poles and exhibiting electromagnetic gauge invariance for both channels. We are now left with soft and collinear singularities that can be absorbed into renormalized non-perturbative functions. 

On the nucleon side, the evolution kernels for transversity $h_1^q(x)$ are well known \cite{sha2024}. It is
\begin{equation}
\begin{aligned}
    h^q_{1,\text{bare}}(x,\mu^2)&= h^q_{1,\text{ren}}(x,\mu^2)+\frac{\alpha_S(\mu^2)}{2\pi}\frac{S_\epsilon}{\epsilon}\left[P^{h_1}_{qq} \otimes h^q_{1,\text{ren}}\right](x,\mu^2),
\end{aligned}
\end{equation}
with splitting function
\begin{equation}
    P^{h_1}_{qq}(y) =C_F\left[\frac{2y}{(1-y)_+}+\frac{3}{2}\delta(1-y)\right].
\end{equation}
On the fragmentation side, it turns out that the $\overline{\text{MS}}$-renormalization of chiral-odd twist-3 fragmentation functions is significantly more involved compared to the one given by the usual leading-twist DGLAP splitting kernels. The evolution is described by a set of coupled integro-differential equations, mixing different twist-3 contributions, and it has been derived in Ref. \cite{Ma_2017}. Crucially, we observe that the collinear singularities emerging from our NLO calculation match exactly with these evolution equations, with the appropriate change of conventions between hadronic matrix elements and variables. It is also very relevant to point out that one could wonder whether the EoMR and LIR relations derived at LO would still hold after renormalization at NLO. In Ref. \cite{Ma_2017} they numerically verified that this is indeed the case, at least for the chiral-odd twist-3 FFs, which is a highly non-trivial result. This means that the twist-3 $\overline{\text{MS}}$-subtraction can be, in principle, performed either \textit{before} or \textit{after} the application of the EoMR.

The corresponding poles can be removed in the $\overline{\text{MS}}$ scheme by introducing renormalized functions
\begin{equation}
\begin{aligned}
    \left[H^q+2zH_1^{\perp(1),q}\right]_{\text{bare}}(z,\mu^2)&=  \left[H^q+2zH_1^{\perp(1),q}\right]_{\text{ren}}(z,\mu^2)\\
    &+\frac{\alpha_S(\mu^2)}{2\pi}\frac{S_\epsilon}{\epsilon}\left[P^{H}_{qqg} \circ \Im \hat{H}_{FU,\text{ren}}^{qg}+P^H_{q\bar{q}q} \circ \Im \hat{H}_{FU,\text{ren}}^{\bar{q}q}\right](z,\mu^2)\\
    \left[E^q\right]_{\text{bare}}(z,\mu^2)&=  \left[E^q\right]_{\text{ren}}(z,\mu^2)\\
    &+\frac{\alpha_S(\mu^2)}{2\pi}\frac{S_\epsilon}{\epsilon}\left[P^{E}_{qqg} \circ \Re \hat{H}_{FU,\text{ren}}^{qg}+P^E_{q\bar{q}q} \circ \Re \hat{H}_{FU,\text{ren}}^{\bar{q}q}\right](z,\mu^2),
\end{aligned}
\end{equation}
where the convolution $\circ$ is a short-hand for
\begin{equation}
    (P\circ H )(z)\equiv\int_z^1\frac{\dd v}{v}\int_0^1\dd\zeta P(v,\zeta) H\left(\frac{z}{v},\zeta\right),
\end{equation}
and the splitting functions are found in Ref. \cite{Ma_2017} OR APPENDIX?. 


\subsection*{Results}
Finally, we present the polarized structure functions for transversely polarized nucleons at NLO accuracy in the strong coupling. We stress that the calculation only involved sub-leading twist effects on the fragmentation side. Therefore, as in the case of the $F_{LT}^{\cos\phi_S}$ structure function, we ignore the $\mathcal{O}(\alpha_S)$ corrections stemming from the nucleon side. Our results read
\begin{equation}\label{eq:F_UT@NLO}
    \begin{aligned}
        F_{UT}^{\sin\phi_S}(x_B,z_h,\mu^2)&=- x_B\frac{4 M_h}{Q}\sum_{q,\bar{q}} e_q^2 \Bigg\{h_1^q(x_B,\mu^2)\frac{\tilde{H}^q(z_h,\mu^2)}{2 z_h}\\
        &+ \frac{\alpha_S(\mu^2)}{2\pi}\int_{x_B}^1\frac{\dd w}{w}\int_{z_h}^1\frac{\dd v}{v}\int_0^1\dd \zeta\,\,h_1^q \left(\frac{x_B}{w},\mu^2\right)\\
        & \times \Bigg[\mathcal{C}_{UT}^{q \to qg}(w,v,\zeta,\mu^2) \frac{\Im \hat{H}_{FU}^{qg}\left(\frac{z_h}{v},\zeta,\mu^2\right)}{1-\zeta}\\
        &\qquad+\mathcal{C}_{UT}^{q \to \bar{q} q}(w,v,\zeta,\mu^2) \Im \hat{H}_{FU}^{\bar{q}q}\left(\frac{z_h}{v},\zeta,\mu^2\right)\Bigg]\Bigg\},
    \end{aligned}
\end{equation}
and
\begin{equation}\label{eq:F_LT@NLO}
    \begin{aligned}
        F_{LT}^{\cos\phi_S}(x_B,z_h,\mu^2)&= +x_B\frac{4 M_h}{Q}\sum_{q,\bar{q}} e_q^2 \Bigg\{
        -h_1^q(x_B,\mu^2)\frac{\tilde{E}^q(z_h,\mu^2)}{2 z_h}\\
        &+ \frac{\alpha_S(\mu^2)}{2\pi}\int_{x_B}^1\frac{\dd w}{w}\int_{z_h}^1\frac{\dd v}{v}\int_0^1\dd \zeta \,\,h_1^q \left(\frac{x_B}{w},\mu^2\right)\\
        &\times\Bigg[\mathcal{C}_{LT}^{q \to qg}(w,v,\zeta,\mu^2)  \frac{\Re \hat{H}_{FU}^{qg}\left(\frac{z_h}{v},\zeta,\mu^2\right)}{1-\zeta}\\
        &\qquad+\mathcal{C}_{LT}^{q \to \bar{q} q}(w,v,\zeta,\mu^2) \Re \hat{H}_{FU}^{\bar{q}q}\left(\frac{z_h}{v},\zeta,\mu^2\right)\Bigg]\\
        &-x_B\frac{M}{M_h} \,g_T^q(x_B,\mu^2)D_1^q(z_h,\mu^2)+\mathcal{O}(\alpha_S) \Bigg\}.
    \end{aligned}
\end{equation}
Crucially, we observe that collinear twist-3 factorization holds at NLO in QCD for these $P_{h\perp}$-integrated observables in SIDIS. Similarly to the unpolarized case, the structure functions are described as convolutions of perturbative hard scattering coefficients and twist-3 fragmentation functions. Here, the convolution involves not only the integrations over $w=x_B/x$ and $v=z_h/z$, but also an extra integral over the partonic variable $\zeta=z/z'$. For the interested reader, we point out that the explicit expressions for the coefficients can be found in Appendix \ref{appendix:polarized}.

The proof that collinear twist-3 factorization at NLO holds, together with the formulae in Eq. \eqref{eq:F_UT@NLO} and \eqref{eq:F_LT@NLO}, is the main result of this work. As pointed out before, this is a non-trivial result. Although some twist-3 NLO calculations have been pursued in the past CITE, they mostly focused on $P_{h\perp}$-weigthed or fully differential SIDIS observables. This is a big difference compared to our calculation, where very few structure functions survive the $P_{h\perp}$ integration. This is also true on a technical point of view, where the treatment of the phase space and the momentum-conserving delta functions are quite different. Furthermore, our calculation is particularly useful in the eye of the upcoming precision colliders such as the EIC, LHCSpin and the JLab $22\,\rm{GeV}$ upgrade. This is because it paves the way to precision calculations of single and double spin asymmetries, which are central observables for the upcoming work of the mentioned experimental collaborations. 
